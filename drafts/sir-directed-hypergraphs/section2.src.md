...
需要强调这只是必要条件，而且远非充分。宇称分类指出哪些量**可能**驱动破缺，却不指出哪一个**实际**驱动——后者是动力学问题。第 VI 节的结论是二者分野明显：破缺由分支结构的 $\mathcal{R}$-奇部分（阶数不对称、双度序列不对称）驱动，而重叠极性 $\Delta\alpha$ 尽管同为 $\mathcal{R}$-奇，其效应在同一测量协议下不可分辨。

但必要条件被满足并不意味着破缺可以在 $\langle S\rangle$ 上被看到。事实上存在一条更强的不变性，它与系综是否 $\mathcal{R}$-不变无关：

> **命题 2.** 设终态可表示为某个随机结构上的可达性——即从种子 $v$ 出发的最终感染集为 $v$ 的出分支 $\mathrm{out}(v)$，如 \cite{kenah2007} 的半有向流行渗流网络所给出。则对**任意**有向超图 $\mathcal{H}$，均匀单种子下的平均爆发规模在方向翻转下不变。
...
破缺出现在可达对**如何分布**，而非其总数。按 \cite{kenah2007}，大爆发的**概率**由巨入分支的相对规模决定，而爆发的**终态规模**由巨出分支决定；$\mathcal{R}$ 恰好交换这两者。因此本文取
...
这决定了第 VI 节破缺实验的设计：全部测量以 $(\Pi,\bar S)$ 为序参量而非 $\langle S\rangle$；主实验驱动分支结构的不对称（阶数与双度序列），$\Delta\alpha$ 的效应作为独立扫描给出上界；$r_{io}$ 扫描与 $\langle S\rangle$ 的不变性分别作为阴性对照与内部一致性检查。
