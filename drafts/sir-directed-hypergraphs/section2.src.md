...
命题 2 的后果是：$\langle S\rangle$ **不能作为方向对称性破缺的序参量**——它在任何结构上都对称，与结构有多不对称无关。破缺出现在可达对**如何分布**，而非其总数。按 \cite{kenah2007}，大爆发的**概率**由巨入分支的相对规模决定，而爆发的**终态规模**由巨出分支决定；$\mathcal{R}$ 恰好交换这两者。因此本文取
