...
需要说明，\eqref{eq:mf} 是我们为本模型构造的均场对照，而非 \cite{li2024directed} 中模型的复述——后者是社会传播而非 SIR，其方向性由单一标量强度参数调节。此处比较的是闭合层级，不是两项工作的模型。
