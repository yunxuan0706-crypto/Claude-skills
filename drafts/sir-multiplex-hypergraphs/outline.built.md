...
> 令 $\widetilde{\mathcal{H}}$ 为把全部 $E_a$ 并起来、每条超边保留其层标签
> $a$（从而保留 $m_a,\theta_a,\beta_a$）所得的**单层带型超图**。则多重超图上的过程与
...
**多元生成函数在此进入，这是层间相关传入动力学的唯一解析通道**：$\Psi$ 的交叉项
（即 $\langle k^{(a)}k^{(b)}\rangle$ 一族）承载全部层间效应。
...
>
> **但要诚实**：上面三步只是论证纲要。对 PRE / SIAM 一级期刊，"树极限下精确"是一句强
> 断言，要么补足严格证明（位形模型的局域弱收敛 + 交换性），要么在正文中降格为
> "数值证据支持、严格性留待后续"。**建议先降格**，等 4.2 的标度结果出来再定。
...
$$
u(i,s)=\frac{\lambda s}{\lambda s+i}\bigl[1+u(i+1,s-1)\bigr]+\frac{i}{\lambda s+i}\,u(i-1,s)\label{eq:cascade}
$$
...
偏差随 $N$ 的标度：闭合应趋零（命题 3.1 的实测确认），均场应趋非零平台。
阈值邻域须单独做有限尺寸标度，不能直接引用有限 $N$ 的值。

### 4.3 阈值的独立测量（**图 3**）
亚临界区单种子簇的平均总规模为 $1/(1-\rho)$，故 $\varepsilon/R(\infty)\to1-\rho(\lambda)$，
在 $\lambda_c$ 处归零。由此外推测 $\lambda_c$，与 $\rho(\mathsf{N})=1$ 比较（给 $\sigma$ 数）。
**全程不使用 $\mathsf{N}$**，是真正独立的测量。
