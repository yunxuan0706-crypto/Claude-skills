...
成对多层已解决 \cite{bianconi2017multilayer,zhao2014routes,barnard2018ebcm}；
高阶单层已解决 \cite{stonge2021ame}；高阶多层只做到渗流
\cite{sunbianconi2021multiplexhg} 或均场且各层角色不同
\cite{fan2022multilayersimplicial,wan2022multilayerho}。本文补上**超出均场的含时
...
> 爆发规模）\cite{ball1997twolevels,pellis2012R0}；带网络结构的重叠版本亦已有
> \cite{ball2009householdnetwork}，并被列为网络流行病学的公开挑战之一
> \cite{pellis2015challenges}。本文与之的区别必须写清楚且只能写这三条：
...
|---|---|
| $M=1$ | 单层超图 SIR 的已知阈值 |
| 全部 $m_a=2$ | $C=T$，\eqref{eq:N} 回到多层网络 SIR 的已知阈值 \cite{bianconi2017multilayer} |
