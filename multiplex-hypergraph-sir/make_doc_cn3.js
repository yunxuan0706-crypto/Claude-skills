...
k.push(caption([
  ...
  R(" 一行始终位于该界之下——成对规模的群体无论 ", { size: SMALL }), ...E("λ", SMALL),
...
k.push(P([R("八组偏差同号（一律为正）这一点本身需要交代：它是系统性的，不能以统计涨落作解。其来源是线性律 (2.9) 的"), R("标度改正项 (correction-to-scaling)", { bold: true }), R("，而非群体闭合与谱方法之间的实质分歧。(2.9) 是 "), ...E("λ→λ_{c}^{−}"), R(" 的渐近陈述，"), ...E("1/χ"), R(" 仅在临界区 (critical region) 内严格线性；拟合窗口一旦离开紧邻区域，次主导项 (subleading term) 即以曲率的形式进入，在 x 截距上留下一个系统偏置。判据是明确的："), R("该偏置应随窗口位置移动而与构型无关", { italics: true }), R("，实质分歧则恰恰相反——应随构型改变而对窗口位置无所谓。实测支持前者：窗口取 "), ...E("[0.85,0.99]λ_{c}"), R(" 时八组偏差一律为负（均值 −1.6σ），取本节所用的 "), ...E("[0.90,0.995]λ_{c}"), R(" 时一律为正（均值 +0.9σ），取 "), ...E("[0.92,0.998]λ_{c}"), R(" 时进一步升至 +2.1σ。偏置随窗口单调变化并在其间过零，各构型的相对次序则始终不变。其量级与 σ 同阶，故在任一固定窗口上都无法与真实偏离相区分。")]));
