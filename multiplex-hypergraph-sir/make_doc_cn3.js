...
k.push(P([R("设节点集为 V，"), ...E("N=|V|"), R("；层 "), ...E("a=1,…,M"), R("，层 a 是 V 上的超图，主线取同质基数 "), ...E("m_{a}"), R("。节点的参与由层度向量 "), ...E("k=(k^{(1)},…,k^{(M)})"), R(" 编码，"), ...E("k^{(a)}"), R(" 为节点所属的层 a 群体数，其联合分布 "), ...E("P(k)"), R(" 是本节唯一的结构输入。群体在其感染成员数达到 "), ...E("θ_{a}"), R(" 时激活，随即以速率 "), ...E("β_{a}=λw_{a}μ"), R(" 向其中每个易感成员独立传递；感染者以速率 μ 康复。以 μ 归一时间后，"), ...E("λ"), R(" 是唯一的标量控制参数（"), ...E("w_{a}"), R(" 为固定的渠道权重）。")], { ind: false }));

k.push(P([R("固定一个检验节点 u 并令其永久保持易感（空腔节点），取 u 所属的一个层 a 群体 e，定义边基变量")]));
k.push(eq("Φ_{a}(t) = Pr[ e 到 t 时刻尚未向 u 传递感染 ]", "2.1"));
k.push(P([R("无向群体和有向超图的分界正在于此。有向情形下尾集和头集不相交，头成员被感染并不延长激活，故"), R("尾成员相互独立", { italics: true }), R("是一条可用的假设；无向群体中每个成员既是传染源也是传染靶，一个成员被感染后群体随即激活，可感染同群其他成员，后者被感染又延长群体的激活时长，剩余成员被感染的概率随之抬高。"), R("群内级联", { bold: true }), R("使逐成员的乘积闭合失效。可行的做法不是忽略群内耦合，而是把它完整吸收进状态：取 e 中除 u 外 "), ...E("m_{a}−1"), R(" 个成员的计数 "), ...E("(s,i,r)"), R("（易感、感染、康复，"), ...E("s+i+r=m_{a}−1"), R("）为房室，定义未传递子概率")], { ind: false }));
...
k.push(P([R("其中 ε 为初始均匀感染比例，Ψ 为节点侧生成函数。其二为"), R("成员外部暴露的独立同分布", { bold: true }), R("：去掉 e 后，其各成员落在互不相交的分支中，故它们经由其他群体被感染的过程相互独立。该假设弱于有向情形下的"), R("尾内无相互作用", { italics: true }), R("——它只断言"), R("外部", { italics: true }), R("暴露独立，群内耦合并未被忽略，而是已由 (2.2) 的计数状态承载。方程组的维数为 "), ...E("Σ_{a}C(m_{a}+1,2)"), R("（S 由 Φ 代数给出、终态规模 "), ...E("R(∞)"), R(" 直接读出，两者均不另设方程），与系统规模 N 无关，积分一次即给出 "), ...E("S(t)"), R(" 与终态规模")], { ind: false }));
...
k.push(P([R("是一个可精确计算的标量。"), ...E("m=2"), R(" 时 "), ...E("C=λ/(1+λ)=T"), R("，级联消失而回到成对传递概率；"), ...E("m≥3"), R(" 时 "), ...E("C>(m−1)T"), R("，"), ...E("λ=1"), R(" 时 "), ...E("m=3,4,5"), R(" 分别超出 11.1%、21.5% 和 31.0%，并非可忽略的修正；"), ...E("θ≥2"), R(" 时单个种子无法激活群体，故 "), ...E("C=0"), R("。")], { ind: false }));

k.push(lead("次代矩阵。", [R("分支过程必须定义在群体层面：每个新感染节点点燃其余各群体，每个被点燃的群体产出 C 个新感染成员，这些成员再点燃各自其余的群体。若改在节点层面朴素地统计谁直接传染了谁，群内级联的后继会被重复计入。按"), R("感染经由哪一层传来", { italics: true }), R("分型，经由层 a 感染的节点其层 b 群体数的期望为 "), ...E("⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩−δ_{ab}"), R("，故")]));
k.push(eq("N_{ab} = C(m_{b},λ_{b},θ_{b})·[ ⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩ − δ_{ab} ],    λ_{b}=λw_{b}", "2.7"));
k.push(P([R("爆发阈值由 "), ...E("ρ(N)=1"), R(" 确定，ρ 为谱半径。标量再生数由此升维为矩阵：单层、多层、成对、高阶都是其特例。所有 "), ...E("m_{a}=2"), R(" 时 "), ...E("C=T"), R("，(2.7) 回到多层网络 SIR 的已知阈值；"), ...E("M=1、m=2"), R(" 时其传递概率阈值 "), ...E("T_{c}=λ_{c}/(1+λ_{c})"), R(" 回到 "), ...E("⟨k⟩/(⟨k^{2}⟩−⟨k⟩)"), R("。另外，N 逐元非负，所以 "), ...E("ρ(N)≥max_{a}N_{aa}"), R("，而 "), ...E("N_{aa}"), R(" 恰为孤立层 a 的分支数：多重超图的阈值因而不高于任一单层的阈值。")], { ind: false }));

// ---------------- 2.2 ----------------
k.push(H2("2.2. 亚临界簇规模与外推判据"));
k.push(P([R("(2.7) 给出的阈值是线性化的产物。本小节给出一条不经线性化的独立判据，其思路和谱方法正相反：始终停在阈值以下，由小种子所引发簇的规模发散读出 "), ...E("λ_{c}"), R("。阈值以下单个种子只引发有限簇，但其平均总规模随 "), ...E("λ→λ_{c}^{−}"), R(" 无界增长。以同一多型分支过程计数，该平均总规模为")], { ind: false }));
...
k.push(lead("有限种子偏置。", [R("(2.9) 是 "), ...E("ε→0"), R(" 的陈述。"), ...E("ε>0"), R(" 时即便恰在 "), ...E("λ_{c}"), R(" 处 "), ...E("R(∞)"), R(" 仍为有限，"), ...E("ε/R(∞)"), R(" 因而不归零，零点被系统性地推高。由于 "), ...E("R(∞)/ε"), R(" 对小 ε 为线性，该偏置可由 "), ...E("ε→0"), R(" 的 Richardson 外推消去，即以 "), ...E("2f(ε/2)−f(ε)"), R(" 代替 "), ...E("f(ε)=R(∞)/ε"), R("。其效果可以直接读出：在几个有代表性的构型上，不作外推时零点一律偏高 0.37%–0.44%，作外推后降到 0.031%–0.065%。偏置的系统性（各构型同号、量级相当），连同该修正的效力，由此一并确认。")]));
...
k.push(P([R("在 "), ...E("m=(3,3)"), R(" 下，对角元 "), ...E("N_{aa}=C(λ)(⟨k^{2}⟩/⟨k⟩−1)=(7/3)C(λ)"), R(" 和 "), ...E("ρ_{12}"), R(" 无关，非对角元 "), ...E("N_{ab}=C(λ)⟨k^{(1)}k^{(2)}⟩/⟨k⟩"), R(" 随 "), ...E("ρ_{12}"), R(" 线性增大，故 "), ...E("ρ(N)=C(λ)(16+ρ_{12})/3"), R("，阈值由 "), ...E("C(λ_{c})=3/(16+ρ_{12})"), R(" 定出——"), R("λ_c 随 ρ_12 单调下降", { bold: true }), R("。图 4 的实线即此曲线："), ...E("λ_{c}"), R(" 从 "), ...E("ρ_{12}=−1"), R(" 处的 0.1062 降到 "), ...E("ρ_{12}=+1"), R(" 处的 0.0930，共约 12.5%。同一族构型在位形模型多重超图上的直接 Gillespie 仿真（由亚临界簇规模外推 "), ...E("1/χ→0"), R("、"), ...E("N=2×10^{4}"), R("）在六个 "), ...E("ρ_{12}"), R(" 上复现该曲线，偏离最大 1.9σ、相对差不超过 1.1%。更值得强调的是其机制：相同节点在两层同时充当枢纽，会在边缘结构不变时集中跨层传播能力、压低阈值；反相关则抬高阈值。")]));
...
k.push(new Paragraph({ alignment: AlignmentType.BOTH, spacing: { after: 180, line: 264 }, indent: { firstLine: 0 },
  children: [
    ...
    R(" 着色，即（a）中各箱之来源。（c）机制：固定 ", { size: SMALL }), ...E("λ", SMALL),
    ...
    R("——即相关参与集中跨层传播能力、压低阈值的机制。", { size: SMALL }),
  ] }));

// ---------------- 2.5 ----------------
k.push(H2("2.5. 数值校验"));
k.push(P([R("校验分四层：闭合内部的自洽、阈值公式的解析锚点、亚临界规模的独立复算，以及级联量本身的蒙特卡洛复核。脚本见 verify.py、verify_ode.py、datacheck.py 和 recheck_mc.py。")], { ind: false }));
...
k.push(lead("蒙特卡洛复核。", [R("级联递推 (2.5) 由单群体的直接 Gillespie 抽样独立复核，该检验既不使用方程组也不使用次代矩阵。在 "), ...E("(m,λ,θ)"), R(" 的八组取值上，"), ...E("2×10^{5}"), R(" 次实现给出的估计和递推值一致，偏离多在一个统计标准误以内；其中 "), ...E("m=2、λ=0.35"), R(" 一组初次测得 3.4σ，以 "), ...E("10^{6}"), R(" 次实现、四个独立随机种子复测后分别为 0.64σ、0.74σ、1.12σ 与 1.03σ，确认该偏离纯属统计涨落而非递推的系统误差。作为整体行为的旁证，"), ...E("ρ(λ)"), R(" 在扫描区间上严格单调递增（由 0.207 增至 2.715），二分因而适定；而以 "), ...E("ε=0.02"), R(" 跨越阈值积分时，"), ...E("R(∞)"), R(" 在阈值以下保持 "), ...E("O(ε)"), R(" 量级、在阈值以上抬升为 "), ...E("O(1)"), R(" 的宏观爆发。")]));

k.push(P([R("四项校验合起来：由亚临界终态外推测得的 "), ...E("λ_{c}"), R(" 与 "), ...E("ρ(N)=1"), R(" 在八组构型上一致到 "), ...E("6.5×10^{−4}"), R("、偏离最大 1.51σ。这是对爆发阈值的一次全非线性独立确认，与前述 Jacobian 复核互补——后者检验线性化谱，前者检验非线性终态所继承的同一阈值。")]));
