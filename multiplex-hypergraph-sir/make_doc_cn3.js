...
k.push(P([R("零点即 "), ...E("λ_{c}"), R("。该判据和 "), ...E("ρ(N)=1"), R(" 不共享任何中间量：一侧把非线性方程组 (2.2)–(2.4) 积分至终态、再对 (2.9) 作线性拟合，另一侧对次代矩阵 (2.7) 求谱半径并二分，二者仅共享模型定义。两条路线的一致因而是对彼此的非平凡检验，也正是图 2 所报告的。")], { ind: false }));
...
k.push(P([R("在 "), ...E("m=(3,3)"), R(" 下，对角元 "), ...E("N_{aa}=C(λ)(⟨k^{2}⟩/⟨k⟩−1)=(7/3)C(λ)"), R(" 和 "), ...E("ρ_{12}"), R(" 无关，非对角元 "), ...E("N_{ab}=C(λ)⟨k^{(1)}k^{(2)}⟩/⟨k⟩"), R(" 随 "), ...E("ρ_{12}"), R(" 线性增大，故 "), ...E("ρ(N)=C(λ)(16+ρ_{12})/3"), R("，阈值由 "), ...E("C(λ_{c})=3/(16+ρ_{12})"), R(" 定出——"), R("λ_c 随 ρ_12 单调下降", { bold: true }), R("。此曲线即图 3(b) 的蓝线："), ...E("λ_{c}"), R(" 从 "), ...E("ρ_{12}=−1"), R(" 处的 0.1062 降到 "), ...E("ρ_{12}=+1"), R(" 处的 0.0930，共约 12.5%。同一族构型在位形模型多重超图上的直接 Gillespie 仿真（由亚临界簇规模外推 "), ...E("1/χ→0"), R("、"), ...E("N=2×10^{4}"), R("）在六个 "), ...E("ρ_{12}"), R(" 上复现该曲线（图 3(b) 箱线），偏离最大 1.9σ、相对差不超过 1.1%。机制明确：正相关使同一节点在两层同时充当度枢纽 (degree hub)——这与 Allard 等 [11] 所讨论的「友谊悖论 (friendship paradox) 同时放大风险与范围」一致——在边缘结构不变时集中了跨层传播能力、压低阈值；反相关则分散枢纽效应、抬高阈值。这一效应在图 3 的相图中表现为临界曲线的整体平移（图 3(c)、(d)）。")]));
