...
k.push(P([R("第 1 节建立的次代矩阵阈值 "), ...E("ρ(N)=1"), R(" 源于线性化分析，其有效性有待一条不经线性化的独立检验。本节即构造这样一条判据，从亚临界区 (subcritical regime) 的终态规模发散行为反向测定阈值，与谱方法互为非平凡检验。该思路与渗流理论 (percolation theory) [1] 中从簇规模分布的临界发散读出渗流阈值 (percolation threshold) 同源。")], { ind: false }));
...
k.push(P([R("无向群体和有向超图 [11] 的分界正在于此。有向情形下传播图 (transmission graph) [11] 的尾集与头集不相交，头成员被感染并不延长激活，故"), R("尾成员相互独立", { italics: true }), R("是一条可用的假设，Allard 等 [11] 的入度 (in-degree) 与出度 (out-degree) 可解耦即建立在这一点上。第 1 节已指出无向群体不具备该性质：一个成员被感染后群体随即激活，可感染同群其他成员；后者被感染又延长群体的激活时长，剩余成员被感染的概率随之抬高。"), R("群内级联", { bold: true }), R("因而使逐成员的乘积闭合失效。本文的策略不是忽略群内耦合（如 MMCA [7,10] 对逐节点状态的独立性假设），而是把耦合完整吸收进群体层面的状态空间：取 e 中除 u 外 "), ...E("m_{a}−1"), R(" 个成员的计数 "), ...E("(s,i,r)"), R("（易感、感染、康复，"), ...E("s+i+r=m_{a}−1"), R("）为房室，其构造类似于 Ding 和 Zhu [9] 的状态转移树 (state transition tree)，但直接建立在连续时间下；据此定义未传递子概率")], { ind: false }));
...
k.push(P([R("其中 ε 为初始均匀感染比例，Ψ 为节点侧生成函数。其二为"), R("成员外部暴露的独立同分布", { bold: true }), R("：去掉 e 后，其各成员落在互不相交的分支中，故它们经由其他群体被感染的过程相互独立。该假设弱于有向情形下的"), R("尾内无相互作用", { italics: true }), R("，只断言"), R("外部", { italics: true }), R("暴露独立；群内耦合并未被忽略，而是已由 (2.2) 的计数状态完整承载。方程组的维数为 "), ...E("Σ_{a}C(m_{a}+1,2)"), R("，与系统规模 N 无关（相较 MMCA [7,10] 的 "), ...E("O(N)"), R(" 维和配对近似 [8] 的 "), ...E("O(N^{2})"), R(" 维，复杂度不随网络增大），积分一次即给出 "), ...E("S(t)"), R(" 与终态规模 (final size)")], { ind: false }));
...
k.push(caption([
  ...
  R(" 平面上的分布（色标经幂律拉伸以展开低值区）；细线为 ", { size: SMALL }),
  ...E("C=2,4", SMALL),
  R(" 等值线，粗线为 ", { size: SMALL }), ...E("C=1", SMALL),
  R(" 等值线，即单个群体已足以自我复制的界。", { size: SMALL }), ...E("m=2", SMALL),
  R(" 一行始终位于该界之下，即成对规模的群体无论 ", { size: SMALL }), ...E("λ", SMALL),
...
k.push(lead("次代矩阵 (next-generation matrix) [15]。", [R("分支过程必须定义在群体层面而非节点层面，与 Allard 等 [11] 在有向传播图上以边为基本单元的计数类似。每个新感染节点点燃其余各群体，每个被点燃的群体产出 C 个新感染成员，这些成员再点燃各自其余的群体。若改在节点层面统计传染链，群内级联的后继会被重复计入。按"), R("感染经由哪一层传来", { italics: true }), R("分型，即引入传播类型 (transmission type)。经由层 a 感染的节点，其层 b 群体数的期望为余度 "), ...E("⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩−δ_{ab}"), R(" [1]，故")]));
k.push(eq("N_{ab} = C(m_{b},λ_{b},θ_{b})·[ ⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩ − δ_{ab} ],    λ_{b}=λw_{b}", "2.7"));
k.push(P([R("爆发阈值由谱半径条件 "), ...E("ρ(N)=1"), R(" 确定。标量再生数 "), ...E("R_{0}"), R(" 由此升维为矩阵，单层网络 [1]、多层网络 [5,6]、成对相互作用与高阶相互作用 [3,4] 均为其特例。极限行为一一对应：所有 "), ...E("m_{a}=2"), R(" 时 "), ...E("C=T"), R("，(2.7) 回到多层网络 SIR 的已知阈值 [5]；"), ...E("M=1"), R("、"), ...E("m=2"), R(" 时传递概率阈值 "), ...E("T_{c}=λ_{c}/(1+λ_{c})"), R(" 回到 Newman [1] 的键渗流公式 "), ...E("⟨k⟩/(⟨k^{2}⟩−⟨k⟩)"), R("。另外，N 逐元非负，Perron–Frobenius 定理给出 "), ...E("ρ(N)≥max_{a}N_{aa}"), R("，而 "), ...E("N_{aa}"), R(" 恰为孤立层 a 的分支数，故多重结构的引入严格降低阈值，与多层网络的一般结论 [5,6] 一致。")], { ind: false }));
...
k.push(lead("有限种子偏置与 Richardson 外推。", [R("(2.9) 是 "), ...E("ε→0"), R(" 的渐近陈述。有限 "), ...E("ε>0"), R(" 时即便恰在 "), ...E("λ_{c}"), R(" 处 "), ...E("R(∞)"), R(" 仍为有限，"), ...E("ε/R(∞)"), R(" 因而不归零，零点被系统性地推高。这是有限种子效应 (finite seed effect)，类似有限尺度标度 (finite-size scaling) [17] 中系统规模对临界点的偏移。由于 "), ...E("R(∞)/ε"), R(" 对小 ε 的展开以线性项为主导，该偏置可由 Richardson 外推 [18] 消去，即以 "), ...E("2f(ε/2)−f(ε)"), R(" 代替 "), ...E("f(ε)=R(∞)/ε"), R("。其效果可以定量验证：在下文八组构型上，不作外推时零点一律偏高 0.35%–0.42%，作外推后降到 0.031%–0.065%，约缩小一个量级。偏置的系统性（各构型同号、量级相当）连同修正的效力由此一并确认。")]));
...
k.push(P([R("八组偏差同号（一律为正）本身需要交代。这是系统性的，不能以统计涨落作解。其来源是线性律 (2.9) 的"), R("标度改正项 (correction-to-scaling)", { bold: true }), R("，而非群体闭合与谱方法之间的实质分歧。(2.9) 是 "), ...E("λ→λ_{c}^{−}"), R(" 的渐近陈述，"), ...E("1/χ"), R(" 仅在临界区 (critical region) 内严格线性；拟合窗口一旦离开紧邻区域，次主导项 (subleading term) 即以曲率的形式进入，在 x 截距上留下一个系统偏置。区分二者的判据在于，"), R("该偏置应随窗口位置移动而与构型无关", { italics: true }), R("，实质分歧则恰恰相反，应随构型改变而对窗口位置无所谓。实测支持前者：窗口取 "), ...E("[0.85,0.99]λ_{c}"), R(" 时八组偏差一律为负（均值 −1.6σ），取本节所用的 "), ...E("[0.90,0.995]λ_{c}"), R(" 时一律为正（均值 +0.9σ），取 "), ...E("[0.92,0.998]λ_{c}"), R(" 时进一步升至 +2.1σ。偏置随窗口单调变化并在其间过零，各构型的相对次序则始终不变。其量级与 σ 同阶，故在任一固定窗口上都无法与真实偏离相区分。")]));
