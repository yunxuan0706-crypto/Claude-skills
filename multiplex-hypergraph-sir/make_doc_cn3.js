...
k.push(P([R("无向群体和有向超图 [11] 的分界正在于此。有向情形下传播图 (transmission graph) [11] 的尾集 (tail set) 和头集 (head set) 不相交，头成员被感染并不延长激活，故"), R("尾成员相互独立", { italics: true }), R("是一条可用的假设。无向群体中每个成员既是传染源也是传染靶，与 Allard 等 [11] 指出的有向传播中入度 (in-degree) 与出度 (out-degree) 可解耦形成对照。一个成员被感染后群体随即激活，可感染同群其他成员；后者被感染又延长群体的激活时长，剩余成员被感染的概率随之抬高。"), R("群内级联 (intra-group cascade)", { bold: true }), R(" 使逐成员的乘积闭合失效。本文的策略不是忽略群内耦合（如 MMCA [7,10] 对逐节点状态的独立性假设），而是把耦合完整吸收进群体层面的状态空间：取 e 中除 u 外 "), ...E("m_{a}−1"), R(" 个成员的计数 "), ...E("(s,i,r)"), R("（易感、感染、康复，"), ...E("s+i+r=m_{a}−1"), R("）为房室，其构造类似于 Ding 和 Zhu [9] 的状态转移树 (state transition tree)，但直接建立在连续时间下；据此定义未传递子概率")], { ind: false }));
