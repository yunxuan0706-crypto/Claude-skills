...
k.push(P([R("经典网络传染模型将接触结构编码为成对边 [1,2]，近年研究逐步推广至高阶相互作用 (higher-order interactions)：单纯复形 [3] 与超图 [4] 上的社会传染已展现出成对模型不具备的不连续相变与双稳态。另一方面，多层网络 (multiplex networks) [5,6] 揭示了渠道间耦合对阈值的非平凡效应。本文的出发点是将两者统一：在多重超图 (multiplex hypergraph) 上建立 SIR 动力学，其核心机制见图 1：多重超图和层度 (a)、单个群体内部的级联 (b)，以及群体层面的分支 (c)。")], { ind: false }));
...
k.push(P([R("节点集为 "), ...E("V"), R("，"), ...E("N=|V|"), R("。层 "), ...E("a=1,…,M"), R("，层 a 是 V 上的均匀超图 (uniform hypergraph) "), ...E("H_{a}=(V,E_{a})"), R("，超边基数 (hyperedge cardinality) "), ...E("|e|=m_{a}"), R("。节点的层度 "), ...E("k^{(a)}(v)=|{e∈E_{a}:v∈e}|"), R("，写成向量 "), ...E("k(v)"), R("。仿照经典网络传染中概率生成函数 (probability generating function, PGF) 的构造 [1,2]，引入两个生成函数：")], { ind: false }));
k.push(eq("Ψ(x)=Σ_{k}P(k)∏_{c}x_{c}^{k_c}", "1.1"));
k.push(eq("ψ_{a}(x)=(1/⟨k^{(a)}⟩)Σ_{k}k_{a}P(k)∏_{c}x_{c}^{k_c−δ_ac}", "1.2"));
k.push(P([...E("Ψ"), R(" 为节点侧度分布的生成函数，"), ...E("ψ_{a}"), R(" 为沿层 a 到达某节点时的余度分布 (excess degree distribution) [1] 的生成函数，指数中的 "), ...E("−δ_{ac}"), R(" 即余度相减：来路那个群体不再计入。联合层度分布 "), ...E("P(k)"), R(" 是模型的唯一结构输入；不同于逐节点的微观马尔可夫链近似 (MMCA) [7,10] 或配对近似 (pair approximation) [8]，本文的闭合在群体层面（而非节点层面）建立消息传递方程，层间效应经由 "), ...E("Ψ"), R(" 和 "), ...E("ψ_{a}"), R(" 的交叉项自然纳入。")], { ind: false }));

// ---------------- 1.2 ----------------
k.push(H2("1.2. SIR 动力学"));
k.push(P([R("节点状态 "), ...E("x_{v}∈{S,I,R}"), R(" 跨层共享。个体在任一渠道被感染，即在所有渠道中都为感染态；这与多层网络中的意识-疫病耦合模型 [9] 不同，后者允许各层携带不同的状态变量。记层 a 的群体 e 中已感染成员数为 "), ...E("n_{e}(t)=|e∩I(t)|"), R("。当 "), ...E("n_{e}≥θ_{a}"), R(" 时群体激活 (activated)，向 e 中每个易感成员各以速率 "), ...E("β_{a}"), R(" 独立传递；激活阈值 "), ...E("θ_{a}"), R(" 反映群体压力效应 (group pressure)，与单纯复形上的社会传染模型 [3] 中的参与阈值一致。感染者以速率 "), ...E("μ"), R(" 康复。易感节点 u 的总风险率为")], { ind: false }));
k.push(eq("Λ_{u}(t)=Σ_{a}Σ_{e∈E_a: u∈e}β_{a}1[n_{e}(t)≥θ_{a}]", "1.3"));
k.push(P([R("以 "), ...E("μ"), R(" 归一时间，定义基本传染强度 "), ...E("λ_{a}=β_{a}/μ"), R("，在 "), ...E("m_{a}=2"), R(" 时退化为经典的传递概率 (transmissibility) "), ...E("T=λ/(1+λ)"), R(" [1]。主线取 "), ...E("θ_{a}=1"), R("。初始时刻每个节点独立地以概率 "), ...E("ε"), R(" 处于感染态、其余易感。与有向超图 [11] 中尾集 (tail set) 和头集 (head set) 不相交因而成员相互独立不同，无向群体中每个成员既是传染源也是传染靶，群内级联 (intra-group cascade) 随之出现，后续分析均围绕它展开。")], { ind: false }));

// ---------------- 1.3 ----------------
k.push(H2("1.3. 消息传递闭合与群体计数方程组"));
k.push(P([R("本节采用空腔方法 (cavity method) [12] 的思路建立消息传递 (message passing) 方程组 [13]，将群内级联完整吸收进群体层面的状态变量。固定一个检验节点 u，令其永久易感（空腔节点）。取一个 u 所属的层 a 群体 e，定义边基变量 (edge-level variable)")], { ind: false }));
...
k.push(P([R("在局域树状假设 (locally tree-like assumption) 下成立。位形模型 (configuration model) [1] 系综 "), ...E("N→∞"), R(" 时局域收敛于超树，短环路 (short cycle) 出现的概率为 "), ...E("O(1/N)"), R("，故 u 所属各群体的上游分支互不相交，节点侧因而闭合为")], { ind: false }));
...
k.push(P([R("和 (1.7) 自洽。初始条件 "), ...E("x^{(a)}_{s,i,0}(0)=C(m_{a}−1,i)(1−ε)^{s}ε^{i}"), R("，其余为零，"), ...E("Φ_{a}(0)=1"), R("。闭合方程组只含各层的群体计数，共 "), ...E("Σ_{a}C(m_{a}+1,2)"), R(" 维，和系统规模 N 无关，"), ...E("M=2"), R("、"), ...E("m=(3,4)"), R(" 时为 16 个；节点侧的 S 由 (1.5) 代数给出，终态规模 "), ...E("R(∞)=1−(1−ε)Ψ(Φ(∞))"), R(" 直接读出，二者都不增维。欲绘出完整时间演化中的 "), ...E("I(t)"), R(" 和 "), ...E("R(t)"), R("（图 3），再附一个求积 "), ...E("Ṙ=μI"), R("（"), ...E("I=1−S−R"), R("）分离即可，它不改变闭合本身的维数。")], { ind: false }));

// ---------------- 1.4 ----------------
k.push(H2("1.4. 次代矩阵与爆发阈值"));
k.push(P([R("经典的基本再生数 (basic reproduction number) "), ...E("R_{0}"), R(" [14] 在多型 (multi-type) 系统中升维为次代矩阵 (next-generation matrix) [15]。定义多型分支过程 (multi-type branching process)：按「感染经由哪一层传来」分型，经由层 a 感染的节点其层 b 群体数的期望为 "), ...E("⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩−δ_{ab}"), R("，每个被点燃的群体产出 "), ...E("C(m_{b},λ_{b},θ_{b})"), R(" 个新感染成员，其中 C 为群内级联的期望产出，由孤立群体上一个小型连续时间 Markov 链的递推精确给出（"), ...E("m=2"), R(" 时退化为成对传递概率 "), ...E("T=λ/(1+λ)"), R(" [1]），其定量刻画与蒙特卡洛校验见 §2.1 及图 2。故次代矩阵")], { ind: false }));
k.push(eq("N_{ab}=C(m_{b},λ_{b},θ_{b})[⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩−δ_{ab}]", "1.10"));
k.push(P([R("爆发阈值由谱半径 (spectral radius) 条件 "), ...E("ρ(N)=1"), R(" 确定。Perron–Frobenius 定理保证 N 的谱半径等于其最大实特征值。在 "), ...E("M=1"), R("、"), ...E("m=2"), R(" 时回到 Newman 的键渗流阈值 [1]。图 3(a) 的控制参数 "), ...E("λ"), R(" 即以此阈值 "), ...E("λ_{c}"), R(" 定标。")], { ind: false }));

// ---------------- 1.5 ----------------
k.push(H2("1.5. 含时演化：闭合与精确仿真"));
k.push(P([R("闭合方程组不仅给出阈值，也复现整个传播过程。图 3(a) 给出 "), ...E("M=2"), R("、"), ...E("m=(3,4)"), R("、"), ...E("N=6000"), R("、"), ...E("λ=1.6λ_{c}"), R(" 下感染比例的完整时间演化，联合层度分布 "), ...E("P(k)"), R(" 在 "), ...E("(2,2)"), R(" 和 "), ...E("(3,3)"), R(" 上各占一半，"), ...E("λ_{c}=0.0993"), R("，"), ...E("ε=0.02"), R("。群体闭合 (1.8) 和精确仿真在 "), ...E("S(t)"), R("、"), ...E("I(t)"), R(" 上的最大偏差分别为 0.0019 和 0.0006，都小于图中线宽；感染峰高相差约 0.3%，峰位 "), ...E("t≈4.4"), R("。含时演化的验证图并入图 3（见第 2 节），与阈值的两条独立测定并列。")], { ind: false }));

// ---------------- 1.6 ----------------
k.push(H2("1.6. 数值校验"));
k.push(P([R("数值仿真采用 Gillespie 精确随机抽样算法 [16] 对 (1.3) 所定义的连续时间 Markov 过程进行精确采样，取 "), ...E("μ=1"), R("。校验分为内部自洽性验证与外部独立复算两部分。")], { ind: false }));
...
k.push(lead("含时复现。", [R("如图 3(a) 所示，群体闭合和精确仿真在整个时间演化上逐点吻合，"), ...E("S(t)"), R(" 和 "), ...E("I(t)"), R(" 的最大偏差都小于线宽（0.0019 和 0.0006），感染峰高相差约 0.3%、峰位都在 "), ...E("t≈4.4"), R("。仿真为 "), ...E("N=6000"), R("、400 次独立实现（脚本 figure2_evolution.py，含 Fenwick 树加速的精确 Gillespie）。残余偏差随系统规模增大单调趋零，属有限尺寸效应，增大系统即可消除。")]));
