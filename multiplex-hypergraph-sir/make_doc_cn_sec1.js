...
k.push(P([R("本模型的三个要素示于图 1：多重超图与层度 (a)、单个群体内部的级联 (b)、以及群体层面的分支 (c)。")], { ind: false }));
// figure 1: mechanism schematic
{
  ...
  k.push(new Paragraph({ alignment: AlignmentType.BOTH, spacing: { after: 180, line: 264 }, indent: { firstLine: 0 },
    children: [
      ...
      R("　多重超图 SIR 群体闭合的机制示意。(a) 多重超图与节点的层度；(b) 单个群体内部的级联，", { size: SMALL }),
...
k.push(P([...E("Ψ"), R(" 描述节点侧，"), ...E("ψ_{a}"), R(" 描述沿层 a 的一个成员槽到达某节点时该节点的余度分布，指数中的 "), ...E("−δ_{ac}"), R(" 即余度相减：来路那个群体不再计入。联合层度分布 "), ...E("P(k)"), R(" 是本模型的结构输入，层间的一切效应最终只经由 "), ...E("Ψ"), R(" 与 "), ...E("ψ_{a}"), R(" 的交叉项进入动力学。")], { ind: false }));

// ---------------- 1.2 ----------------
k.push(H2("1.2. SIR 动力学"));
k.push(P([R("节点状态 "), ...E("x_{v}∈{S,I,R}"), R(" 跨层共享——个体在任一渠道被感染即在所有渠道中均为感染态。记层 a 的群体 e 中已感染成员数为 "), ...E("n_{e}(t)=|e∩I(t)|"), R("。当 "), ...E("n_{e}≥θ_{a}"), R(" 时群体激活，向 e 中每个易感成员各以速率 "), ...E("β_{a}"), R(" 独立传递，否则静默；感染者以速率 "), ...E("μ"), R(" 康复。易感节点 u 的总风险率")], { ind: false }));
k.push(eq("Λ_{u}(t)=Σ_{a}Σ_{e∈E_a: u∈e}β_{a}1[n_{e}(t)≥θ_{a}]", "1.3"));
k.push(P([R("以 "), ...E("μ"), R(" 归一时间，"), ...E("λ_{a}=β_{a}/μ"), R("，主线取 "), ...E("θ_{a}=1"), R("。初始时刻每个节点独立地以概率 "), ...E("ε"), R(" 处于感染态、其余易感，"), ...E("ε"), R(" 即均匀初始感染比例。与有向超图不同，无向群体中每个成员既是传染源也是传染靶：一个成员被感染后群体激活，可感染同群其他成员，后者被感染又延长群体的激活时长，形成群内级联。本模型的新内容均源于此。")], { ind: false }));
...
k.push(P([R("一个易感成员的总感染率是外部与群内之和 "), ...E("h_{a}+β_{a}1[i≥θ_{a}]"), R("；群体向 u 的传递以速率 "), ...E("β_{a}1[i≥θ_{a}]"), R(" 把概率移出「未传递」类。据此")], { ind: false }));
k.push(...eqBreak(
...
k.push(P([R("（越界下标记为零。）对 (1.8) 求和，"), ...E("h_{a}"), R(" 项、群内项与 "), ...E("μ"), R(" 项各自守恒相消，只余")], { ind: false }));
k.push(eq("Φ̇_{a}=−β_{a}Φ^{A}_{a}", "1.9"));
k.push(P([R("与 (1.7) 自洽。初始条件 "), ...E("x^{(a)}_{s,i,0}(0)=C(m_{a}−1,i)(1−ε)^{s}ε^{i}"), R("，其余为零，"), ...E("Φ_{a}(0)=1"), R("。闭合方程组只含各层的群体计数，共 "), ...E("Σ_{a}C(m_{a}+1,2)"), R(" 维，与系统规模 N 无关，"), ...E("M=2"), R("、"), ...E("m=(3,4)"), R(" 时为 16 个；节点侧的 S 由 (1.5) 代数给出，终态规模 "), ...E("R(∞)=1−(1−ε)Ψ(Φ(∞))"), R(" 直接读出，二者均不增维。欲绘出完整时间演化中的 "), ...E("I(t)"), R(" 与 "), ...E("R(t)"), R("（图 2），再附一个求积 "), ...E("Ṙ=μI"), R("（"), ...E("I=1−S−R"), R("）分离即可，它不改变闭合本身的维数。")], { ind: false }));

// ---------------- 1.4 ----------------
k.push(H2("1.4. 爆发阈值"));
k.push(P([R("以「感染经由哪一层传来」为型，经由层 a 感染的节点其层 b 群体数的期望为 "), ...E("⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩−δ_{ab}"), R("，每个被点燃的群体产出 "), ...E("C(m_{b},λ_{b},θ_{b})"), R(" 个新感染成员（C 为孤立群体内的群内级联期望，由一个小型连续时间 Markov 链的递推给出，"), ...E("m=2"), R(" 时退化为成对传递概率）。故多型分支矩阵")], { ind: false }));
k.push(eq("N_{ab}=C(m_{b},λ_{b},θ_{b})[⟨k^{(a)}k^{(b)}⟩/⟨k^{(a)}⟩−δ_{ab}]", "1.10"));
k.push(P([R("爆发阈值由谱半径条件 "), ...E("ρ(N)=1"), R(" 确定。图 2 的控制参数 "), ...E("λ"), R(" 即以此阈值 "), ...E("λ_{c}"), R(" 定标。")], { ind: false }));

// ---------------- 1.5 ----------------
k.push(H2("1.5. 含时演化：闭合、均场与精确仿真"));
k.push(P([R("作为对照，同一模型的度均场取群体激活概率为系综平均 "), ...E("Θ_{a}=1−(1−φ_{a})^{m_a−1}"), R("，其中 "), ...E("φ_{a}"), R(" 为层 a 成员槽被感染节点占据的比例，")], { ind: false }));
k.push(eq("ṡ_{k}=−s_{k}Σ_{a}k_{a}β_{a}Θ_{a},    i̇_{k}=s_{k}Σ_{a}k_{a}β_{a}Θ_{a}−μ i_{k}", "1.11"));
k.push(P([R("均场丢弃了群内相关、邻域相关与群内级联，是评价群体闭合的基线。")], { ind: false }));
k.push(P([R("图 2 给出 "), ...E("M=2"), R("、"), ...E("m=(3,4)"), R("、"), ...E("N=6000"), R("、"), ...E("λ=1.6λ_{c}"), R(" 下的完整时间演化，联合层度分布 "), ...E("P(k)"), R(" 在 "), ...E("(2,2)"), R(" 与 "), ...E("(3,3)"), R(" 上各占一半，"), ...E("λ_{c}=0.0993"), R("，"), ...E("ε=0.02"), R("。群体闭合 (1.8) 与精确仿真在 "), ...E("S(t)"), R(" 与 "), ...E("I(t)"), R(" 上的最大偏差分别为 0.0039 与 0.0011，均小于图中线宽；感染峰高相差 0.9%。度均场 (1.11) 则把感染峰值由仿真的 0.086 抬到 0.139（1.6 倍），并把峰位由 "), ...E("t≈4.4"), R(" 提前到 "), ...E("t≈3.6"), R("。群体闭合复现了完整的含时演化，度均场在同一参数下显著失真。")], { ind: false }));
// figure 2: time evolution
{
  const img = fs.readFileSync("fig2_evolution.png");
  k.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 160, after: 70 }, indent: { firstLine: 0 },
    children: [new ImageRun({ type: "png", data: img, transformation: { width: 560, height: 243 } })] }));
  k.push(new Paragraph({ alignment: AlignmentType.BOTH, spacing: { after: 180, line: 264 }, indent: { firstLine: 0 },
    children: [
      ...
      R("。黑线为 400 次独立 Gillespie 实现的均值，灰带为 95% 置信区间，最宽处约与线宽相当；蓝虚线为群体闭合 (1.8)，橙点线为度均场 (1.11)。(a) 易感比例 ", { size: SMALL }),
...
k.push(lead("内部自洽。", [R("方程组自带两条无需仿真的自检。其一为求和规则：对 (1.8) 逐态求和应严格给出 (1.9)，残差处于机器精度（"), ...E("10^{−20}"), R(" 量级），说明它是方程组的代数恒等式。其二为全体成员易感的恒等式 "), ...E("x^{(a)}_{m_a−1,0,0}=[(1−ε)ψ_{a}(Φ)]^{m_a−1}"), R("，其残差随积分步长按 "), ...E("dt^{4}"), R(" 收敛（dt 由 0.02 逐次减半时残差为 "), ...E("1.3×10^{−10}"), R("、"), ...E("8.0×10^{−12}"), R("、"), ...E("5.0×10^{−13}"), R("、"), ...E("3.1×10^{−14}"), R("，相邻比约 16），与四阶 Runge–Kutta 的阶一致，说明该恒等式是方程组的严格推论、残差纯属积分误差。")]));
k.push(lead("阈值自洽。", [R("阈值公式 (1.10) 与方程组自身在无病定态的线性失稳独立比对：将 (1.8) 作数值 Jacobian、对最大实部特征值二分求阈值，与 "), ...E("ρ(N)=1"), R(" 在六组构型（含反相关度分布、非对称速率与群体规模、三层、以及成对退化情形 "), ...E("λ_{c}=1/3"), R("）上的相对差均在 "), ...E("10^{−9}"), R(" 量级，受数值失稳判据的分辨率所限（Jacobian 取中心差分）。两条路径不共享代码；若以 "), ...E("(m−1)T"), R(" 取代群内级联 C，该一致性立即消失。")]));
k.push(lead("含时复现。", [R("如图 2 所示，群体闭合与精确仿真在整个时间演化上逐点吻合，"), ...E("S(t)"), R(" 与 "), ...E("I(t)"), R(" 的最大偏差均小于线宽（0.0039 与 0.0011），感染峰高相差 0.9%；对峰邻域作抛物线插值以消除网格离散后，闭合与仿真的峰位分别为 "), ...E("t=4.38"), R(" 与 4.37，相差约 0.01；度均场则系统性偏离。偏差随系统规模的标度进一步表明，群体闭合的误差随 N 增大单调趋零（至 "), ...E("N=8000"), R(" 已落入蒙特卡洛噪声），而度均场的误差收敛到非零平台——两者不是同一类误差：前者可靠增大系统消除，后者是闭合层级的误差，不能。")]));
