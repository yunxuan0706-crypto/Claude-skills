...
k.push(P([R("节点状态 "), ...E("x_{v}∈{S,I,R}"), R(" 跨层共享——个体在任一渠道被感染即在所有渠道中都为感染态，这与多层网络中的意识-疫病耦合模型 [9] 不同，后者允许各层携带不同的状态变量。记层 a 的群体 e 中已感染成员数为 "), ...E("n_{e}(t)=|e∩I(t)|"), R("。当 "), ...E("n_{e}≥θ_{a}"), R(" 时群体激活 (activated)，向 e 中每个易感成员各以速率 "), ...E("β_{a}"), R(" 独立传递；激活阈值 "), ...E("θ_{a}"), R(" 反映群体压力效应 (group pressure)，与单纯复形上的社会传染模型 [3] 中的参与阈值一致。感染者以速率 "), ...E("μ"), R(" 康复。易感节点 u 的总风险率为")], { ind: false }));
k.push(eq("Λ_{u}(t)=Σ_{a}Σ_{e∈E_a: u∈e}β_{a}1[n_{e}(t)≥θ_{a}]", "1.3"));
k.push(P([R("以 "), ...E("μ"), R(" 归一时间，定义基本传染强度 "), ...E("λ_{a}=β_{a}/μ"), R("——它在 "), ...E("m_{a}=2"), R(" 时退化为经典的传递概率 (transmissibility) "), ...E("T=λ/(1+λ)"), R(" [1]。主线取 "), ...E("θ_{a}=1"), R("。初始时刻每个节点独立地以概率 "), ...E("ε"), R(" 处于感染态、其余易感。与有向超图 [11] 中尾集 (tail set) 和头集 (head set) 不相交因而成员相互独立不同，无向群体中每个成员既是传染源也是传染靶，群内级联 (intra-group cascade) 随之出现，后续分析均围绕它展开。")], { ind: false }));
...
k.push(P([R("在局域树状假设 (locally tree-like assumption) 下——位形模型 (configuration model) [1] 系综 "), ...E("N→∞"), R(" 时局域收敛于超树，短环路 (short cycle) 出现的概率为 "), ...E("O(1/N)"), R("——u 所属各群体的上游分支互不相交，节点侧因而闭合为")], { ind: false }));
