...
k.push(P([R("阈值公式 (2.7) 把结构压缩成两样东西：联合层度分布的二阶矩，以及各层的群体基数。压缩得越狠，可问的问题就越具体——哪些结构自由度真的移动阈值，哪些被压掉了。本节先把阈值画成 "), ...E("(λ_{1},λ_{2})"), R(" 平面上的相图、揭示协同区（图 3），再沿两条结构方向扫描可见的自由度（图 4a、b），最后用一条公式看不见的自由度检验公式本身（图 4c、d）。")], { ind: false }));

// ---------------- 3.1 ----------------
k.push(H2("3.1. 协同区：两条弱渠道合并后的超临界"));
k.push(P([R("此前各层的速率由一个标量 "), ...E("λ"), R(" 经固定权重 "), ...E("w_{a}"), R(" 统一定标。放开约束，令两层各自的速率 "), ...E("λ_{1}"), R(" 与 "), ...E("λ_{2}"), R(" 独立变化，阈值条件 "), ...E("ρ(N)=1"), R(" 就不再是一个点，而是 "), ...E("(λ_{1},λ_{2})"), R(" 平面上的一条曲线。与之对照的是两条单层条件 "), ...E("N_{aa}=1"), R("，即各层若被孤立出来自行点燃所需的速率。三条线围出一个楔形")], { ind: false }));
k.push(eq("𝒮={(λ_{1},λ_{2}): N_{11}<1, N_{22}<1, ρ(N)>1}", "3.1"));
k.push(P([R("楔内每一点都对应同一个局面：任一渠道单独存在都不足以引发爆发，两者并存却足够。图 3(a) 以 "), ...E("P={(2,2),(3,3)}"), R("、"), ...E("m=(3,3)"), R(" 的谱半径热力图画出该楔形，白色等值线为 "), ...E("ρ(N)"), R(" 的等高线。单层各自的阈值为 "), ...E("λ_{c}^{(a)}=0.400567"), R("，两层对称合并后降到 0.128162，低 68.0%；§2.5 用作解析锚点的 "), ...E("λ_{1}=λ_{2}=0.13"), R("（"), ...E("N_{11}=N_{22}=0.3860"), R("、"), ...E("ρ=1.0132"), R("）正落在楔内紧贴边界处，图中以菱形标出。楔形并非临界线附近的窄缝，而是占据了整个 "), ...E("[0,λ_{c}^{(a)}]^{2}"), R(" 方块中临界曲线以上的全部区域。")]));
k.push(P([R("楔形的宽窄由层间耦合决定。取 §2.4 的 "), ...E("{2,4}"), R(" 族：边缘分布固定意味着两条单层线被钉死，"), ...E("ρ_{12}"), R(" 只能移动临界曲线本身。曲线随 "), ...E("ρ_{12}"), R(" 由 −1 增到 +1 而整体内移（图 3c），对称阈值由 0.106203 降到 0.092956，与 §2.4 的两个端点逐位相同；图 3(d) 把移动量画成"), ...E("Δλ_{2,c}(λ_{1},ρ_{12})"), R(" 的完整曲面。"), R("协同是多重结构的默认状态", { bold: true }), R("：只要两层非完全孤立，合并阈值严格低于任一单层——§2.1 由 "), ...E("ρ(N)≥max_{a}N_{aa}"), R(" 给出下界，图 3 在整个平面上印证。图 3(b) 另将 "), ...E("λ_{c}(ρ_{12})"), R(" 的精确曲线与位形模型上的直接 Gillespie 仿真自助分布对照，六点一致至 1.9σ。")]));
k.push(figure("figure3_phase.png", 452, 380));
k.push(caption([
  R("图 3", { bold: true, size: SMALL }),
  R("　爆发阈值的相图（核心结果）。(a) ", { size: SMALL }), ...E("P={(2,2),(3,3)}", SMALL), R("、", { size: SMALL }), ...E("m=(3,3)", SMALL),
...
k.push(lead("最劣配置。", [R("固定总传播预算 "), ...E("Σ_{a}w_{a}"), R("，在两层之间转移权重，问哪种分配最危险。答案在四组构型上一致："), R("最劣配置总是内点", { bold: true }), R("，从不是把预算全部押在最强的那条渠道上。理由可以从 (2.7) 直接读出——纯配置令 "), ...E("λ_{b}=0"), R(" 的那一列整列归零，"), R("把 N 的非对角元一并关掉", { bold: true }), R("，而跨层项恰是多重结构的全部收益所在。图 4(a) 以各构型自身最优纯配置为单位作归一：对称的 "), ...E("m=(3,3)"), R("、"), ...E("k=(3,3)"), R(" 在 "), ...E("w_{1}=0.5"), R(" 处取到最劣，阈值比最优纯配置低 29.3%；非对称的 "), ...E("m=(2,5)"), R("、"), ...E("k=(4,2)"), R(" 最劣点移到 "), ...E("w_{1}=0.32"), R("，低 13.8%。即便层间差异大到最劣点几乎贴住某一端（"), ...E("m=(3,5)"), R(" 时 "), ...E("w_{1}=0.079"), R("，仅低 0.45%），它仍严格落在内部。此问题为多层所特有：单层没有可供分配的自由度。")]));
k.push(lead("群体粒度。", [R("「少数大群体」与「多数小群体」孰更危险，在问清"), R("固定什么", { italics: true }), R("之前没有答案，而两种自然的固定方式给出相反的结论（图 4b 把两者叠在一张图上）。固定每节点的群体数 "), ...E("k=4"), R("，阈值从 "), ...E("m=2"), R(" 的 0.500000 一路降到 "), ...E("m=8"), R(" 的 0.044149，大群体危险得多，与 "), ...E("C"), R(" 关于 "), ...E("m"), R(" 超线性增长的预期一致。改为固定接触预算 "), ...E("k(m−1)=12"), R("，即每节点可及的邻居数相同，结论"), R("反转", { bold: true }), R("：阈值从 "), ...E("m=2"), R(" 的 0.100000 升到 "), ...E("m=7"), R(" 的 0.148224，"), R("多数小群体反而更危险", { bold: true }), R("。")]));
k.push(P([R("反转的来源是单层阈值条件本身")], { ind: false }));
k.push(eq("(k−1)·C(m,λ_{c},θ)=1", "3.2"));
k.push(P([R("群体层面的分支因子是余度与级联的乘积。增大 "), ...E("m"), R(" 确实换来 "), ...E("C"), R(" 的超线性增益——固定接触预算时 "), ...E("C(λ_{c})"), R(" 由 0.0909 一路升到 1.0000——但同时付出余度 "), ...E("k−1"), R(" 由 11 坍缩到 1 的代价，后者取胜。极端情形最能说明问题："), ...E("k=1"), R(" 时 "), ...E("k−1=0"), R("，无论群体多大，该层单独都永远无法传播，因为被感染者没有第二个群体可以传出去。"), R("余度坍缩否证了「大群体更危险」的朴素预期", { bold: true }), R("：该预期只在固定 "), ...E("k"), R(" 时成立，一旦按可及邻居数作公平比较便告失效（"), ...E("k=1"), R(" 时永不传播，未绘于图 4b）。")]));

// ---------------- 3.3 ----------------
k.push(H2("3.3. 层间重叠：一条可证伪的推论"));
k.push(P([R("以上扫描都在公式看得见的自由度上进行。检验理论边界须用一条它"), R("看不见", { italics: true }), R("的自由度。(2.7) 只读取联合层度分布与群体基数，因此两个 "), ...E("P(k)"), R(" 与 "), ...E("m"), R(" 相同的多重超图必被赋予同一个 "), ...E("λ_{c}"), R("，无论两层的群体在节点上如何相对摆放。定义层间重叠")], { ind: false }));
k.push(eq("o=#{节点对{u,v}: 同处某层1群体 ∧ 同处某层2群体}/#{节点对{u,v}: 同处某层1群体}", "3.3"));
k.push(P([R("一个被重复使用的节点对恰好闭合一条 4-循环，"), R("恰为局域树状假设所丢弃", { bold: true }), R("，由此给出一条可证伪的推论：固定 "), ...E("P(k)"), R("、"), ...E("m"), R(" 与 "), ...E("θ"), R("，只改 "), ...E("o"), R("，则 "), ...E("λ_{c}"), R(" 必须不动。若测得阈值随 "), ...E("o"), R(" 移动，移动的幅度就是树状闭合失效的定量刻度。")]));
...
k.push(P([R("推论因此被干净地证伪。"), ...E("o=1"), R(" 处偏离水平线达 " + pullEnds + "σ，逐点递增也都显著（相邻两点之差最小为 4.5σ，全部同号）。"), R("层间重叠并非高阶修正，其调控力度远超层间参与相关", { bold: true }), R("：图 3 中 "), ...E("ρ_{12}"), R(" 全程只移动阈值 12.5%，"), ...E("o"), R(" 却移动了 " + rise + "%，相差近一个数量级。")], { ind: false }));
k.push(P([R("失效的方向也符合机制。重叠把传播投向已经可及的节点，"), R("同一条边被两层重复计入", { bold: true }), R("，而 (2.7) 按互不相交的分支计数，于是系统性地高估了分支能力、低估了阈值。图 4(d) 给出小 "), ...E("λ"), R(" 下的展开：理论按 "), ...E("3C(3,λ)≈6λ"), R(" 计数，"), ...E("o=1"), R(" 的实际过程只有 "), ...E("C(3,2λ)≈4λ"), R("，比值 3:2 即高估的量级。"), R("树状闭合的适用边界由此定量给出", { bold: true }), R("：它要求层间群体近乎不相交，位形模型系综自动满足（"), ...E("o=O(1/N)"), R("），而任何具有实质层间重叠的真实系统都会落在公式之外。")]));
