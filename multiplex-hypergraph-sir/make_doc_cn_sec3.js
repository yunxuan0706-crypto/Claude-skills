...
k.push(P([R("楔内每一点都对应同一个局面：任一渠道单独存在都不足以引发爆发，两者并存却足够。图 3(a) 以 "), ...E("P={(2,2),(3,3)}"), R("、"), ...E("m=(3,3)"), R(" 的谱半径热力图画出该楔形，白色等值线为 "), ...E("ρ(N)"), R(" 的等高线。单层各自的阈值为 "), ...E("λ_{c}^{(a)}=0.400567"), R("，两层对称合并后降到 0.128162，低 68.0%；§2.5 用作解析锚点的 "), ...E("λ_{1}=λ_{2}=0.13"), R("（"), ...E("N_{11}=N_{22}=0.3860"), R("、"), ...E("ρ=1.0132"), R("）正落在楔内紧贴边界处，图中以菱形标出。楔形之大值得留意：它并非临界线附近的一条窄缝，而是占据了整个 "), ...E("[0,λ_{c}^{(a)}]^{2}"), R(" 方块中临界曲线以上的全部区域。")]));
k.push(P([R("楔形的宽窄由层间耦合决定。取 §2.4 的 "), ...E("{2,4}"), R(" 族：边缘分布固定意味着两条单层线被钉死，"), ...E("ρ_{12}"), R(" 只能移动临界曲线本身。曲线随 "), ...E("ρ_{12}"), R(" 由 −1 增到 +1 而整体内移（图 3c），对称阈值由 0.106203 降到 0.092956，与 §2.4 的两个端点逐位相同；图 3(d) 把这一移动画成 "), ...E("Δλ_{2,c}(λ_{1},ρ_{12})"), R(" 的完整曲面。"), R("协同不是可有可无的修饰，而是多重结构的默认状态", { bold: true }), R("：只要两层都不是完全孤立，合并阈值就严格低于任一单层阈值——这一点在 §2.1 已由 "), ...E("ρ(N)≥max_{a}N_{aa}"), R(" 给出，图 3 把它在整个平面上兑现。图 3(b) 另将 "), ...E("λ_{c}(ρ_{12})"), R(" 的精确曲线与位形模型上的直接 Gillespie 仿真自助分布对照，六点一致至 1.9σ。")]));
...
k.push(P([R("群体层面的分支因子是余度与级联的乘积。增大 "), ...E("m"), R(" 确实换来 "), ...E("C"), R(" 的超线性增益——固定接触预算时 "), ...E("C(λ_{c})"), R(" 由 0.0909 一路升到 1.0000——但同时付出余度 "), ...E("k−1"), R(" 由 11 坍缩到 1 的代价，后者取胜。极端情形最能说明问题："), ...E("k=1"), R(" 时 "), ...E("k−1=0"), R("，无论群体多大，该层单独都永远无法传播，因为被感染者没有第二个群体可以传出去。"), R("这一条否证了「大群体更危险」的朴素预期", { bold: true }), R("：该预期只在固定 "), ...E("k"), R(" 时成立，一旦按可及邻居数作公平比较便告失效（"), ...E("k=1"), R(" 时永不传播，未绘于图 4b）。")]));
...
k.push(P([R("推论因此被干净地证伪。"), ...E("o=1"), R(" 处偏离水平线达 " + pullEnds + "σ，逐点递增也都显著（相邻两点之差最小为 4.5σ，全部同号）。"), R("层间重叠不是可以忽略的高阶修正，而是比层间参与相关更强的阈值调控量", { bold: true }), R("——图 3 中 "), ...E("ρ_{12}"), R(" 扫遍全程只移动阈值 12.5%，此处 "), ...E("o"), R(" 却移动了 " + rise + "%，相差近一个数量级。")], { ind: false }));
