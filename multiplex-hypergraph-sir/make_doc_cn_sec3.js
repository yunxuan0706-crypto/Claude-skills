...
k.push(P([R("次代矩阵 (2.7) 将网络结构压缩为两个充分统计量 (sufficient statistics)：联合层度分布的二阶矩 "), ...E("⟨k^{(a)}k^{(b)}⟩"), R("，以及各层的超边基数 "), ...E("m_{a}"), R("。这种低维约化 (dimensional reduction) 赋予理论以预测力，但同时限定了其适用范围，被约化丢弃的结构自由度即构成理论的盲区。本节从三个方向系统考察阈值的结构依赖：(i) 将阈值推广至 "), ...E("(λ_{1},λ_{2})"), R(" 平面上的相图，揭示多层协同效应 (synergy)（图 4）；(ii) 在理论可见的自由度内扫描渠道配置与群体粒度效应（图 5a、b）；(iii) 利用理论不可见的自由度，即层间重叠 (inter-layer overlap)，对树状闭合假设本身进行可证伪检验 (falsifiable test)（图 5c、d）。")], { ind: false }));

// ---------------- 3.1 ----------------
k.push(H2("3.1. 协同区：多层耦合的超临界相变"));
k.push(P([R("两条各自亚临界的传播渠道合并之后能否引发宏观爆发，是跨层耦合研究的核心问题之一 [5,6]。此前各层的速率由一个标量 "), ...E("λ"), R(" 经固定权重 "), ...E("w_{a}"), R(" 统一定标。放开约束，令两层各自的速率 "), ...E("λ_{1}"), R(" 与 "), ...E("λ_{2}"), R(" 独立变化，阈值条件 "), ...E("ρ(N)=1"), R(" 升维为 "), ...E("(λ_{1},λ_{2})"), R(" 参数平面上的一条临界曲线 (critical curve)。与之对照的是两条单层条件 "), ...E("N_{aa}=1"), R("，即各层孤立时各自的渗流阈值 [1]。三条线围出协同区")], { ind: false }));
...
k.push(lead("群体粒度反转 (granularity reversal)。", [R("高阶相互作用中的一个基本问题，是「少数大群体」与「多数小群体」孰更有利于传播 [3,4]。答案取决于归一化方式，在问清"), R("固定什么", { italics: true }), R("之前并无唯一答案。两种自然的归一化给出定性相反的结论（图 5b 叠示二者）。固定每节点的群体参与度 (membership) "), ...E("k=4"), R("，阈值从 "), ...E("m=2"), R(" 的 0.500000 单调降至 "), ...E("m=8"), R(" 的 0.044149，大群体确实更危险，与群内级联 "), ...E("C"), R(" 关于 "), ...E("m"), R(" 的超线性增长一致。改为固定接触预算 (contact budget) "), ...E("k(m−1)=12"), R("，即每节点可及的邻居总数不变，结论"), R("反转", { bold: true }), R("：阈值从 "), ...E("m=2"), R(" 的 0.100000 升到 "), ...E("m=7"), R(" 的 0.148224，"), R("多数小群体反而更危险", { bold: true }), R("。")]));
...
k.push(P([R("以上扫描都在次代矩阵 (2.7) 可见的自由度上进行。任何理论的可信度不仅取决于它在已知极限下的正确性，更取决于它在何处失效；对理论边界的定量刻画，正是 Popper 意义上可证伪性 (falsifiability) 的体现。(2.7) 的输入仅为联合层度分布 "), ...E("P(k)"), R(" 与超边基数 "), ...E("m"), R("，因此两个 "), ...E("P(k)"), R(" 与 "), ...E("m"), R(" 相同的多重超图必被赋予同一个 "), ...E("λ_{c}"), R("，无论两层的超边在节点上如何相对排列。定义层间重叠 (inter-layer overlap)")], { ind: false }));
...
k.push(lead("误差的标定。", [R("截距的标准误由 delta 方法给出，其前提是线性律在整个窗口内严格成立。凡残余曲率未被消尽之处，"), ...E("χ^{2}/dof"), R(" 便大于 1，该标准误偏乐观，按惯例应以 "), ...E("√(χ^{2}/dof)"), R(" 放大。六条拟合中四条的 "), ...E("χ^{2}/dof"), R(" 在 1 附近而无需放大，"), ...E("o=1"), R(" 一条为 4.7（放大 " + facMax + " 倍），其窗口跨越最宽的绝对 "), ...E("λ"), R(" 区间，保留的曲率也最多。以下所有显著性均取放大后的误差。")]));
