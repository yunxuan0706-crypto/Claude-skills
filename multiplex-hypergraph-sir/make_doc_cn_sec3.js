...
k.push(lead("净偏差变号。", [R("阈值处的图像并不能推广到整个 "), ...E("(m,λ)"), R(" 平面。图 3(b) 给出净偏差在该平面上的符号与量级：绝大部分区域为暖色，即均场高估 ρ；但在"), R("大群体、中等 λ", { bold: true }), R(" 的一片区域内 (3.5) 越过 1，级联项压过前两项，"), R("均场转为低估 ρ", { bold: true }), R("。对本构型，变号自 "), ...E("m=8"), R(" 起出现，窗口为 "), ...E("λ∈[0.086,0.394]"), R("，并随 m 加宽，"), ...E("m=16"), R(" 处 "), ...E("ρ/ρ^{MF}"), R(" 最大达 1.58。变号的"), R("起点依赖于度分布", { italics: true }), R("——它由 "), ...E("f_{D}"), R(" 定标。本文诸构型的 X 元素皆正，减去单位阵使全部特征值平移 −1，故 "), ...E("f_{D}=1−1/ρ(X)"), R(" 精确成立：度越异质、"), ...E("ρ(X)"), R(" 越大，"), ...E("f_{D}"), R(" 越接近 1，D 项的高估越弱，变号也越早。六组分布上 "), ...E("ρ(X)"), R(" 依次为 5.00、6.00、8.25、8.50、20.0 与 22.9，对应起点依次为 "), ...E("m=9"), R("、9、8、8、6 与 6，逐一单调对应（"), ...E("k∈{1,3}"), R(" 与 3-正则为 9，"), ...E("k∈{3,5}"), R(" 独立与相关为 8，10-正则与重尾 "), ...E("k∈{2,20}"), R(" 为 6）——但变号的"), R("存在性并不依赖", { italics: true }), R("：六组分布无一例外。")]));
...
k.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 160, after: 70 }, indent: { firstLine: 0 },
  children: [new ImageRun({ type: "png", data: img, transformation: { width: 468, height: 360 } })] }));
k.push(new Paragraph({ alignment: AlignmentType.BOTH, spacing: { after: 180, line: 264 }, indent: { firstLine: 0 },
  children: [
    ...
    R(" 随 m 的变化，恒小于 1。(d) 精确仿真校验，纵轴为归一化的平均簇规模 ", { size: SMALL }),
    ...E("χ/χ^{exact}", SMALL), R("（对数标度），横虚线 ", { size: SMALL }), ...E("y=1", SMALL),
    R(" 即精确闭合；蓝色标记为多重超图上的 Gillespie 抽样（", { size: SMALL }),
    ...E("N=4×10^{4}", SMALL), R("，误差棒为标准误），橙线为同一参数下度均场的预言，圆与方分别对应 ", { size: SMALL }),
    ...E("λ=0.6λ_{c}", SMALL), R(" 与 ", { size: SMALL }), ...E("0.8λ_{c}", SMALL),
    R("。仿真与精确理论一致至 ", { size: SMALL }), ...E("1.3σ", SMALL),
    R("，而均场被排除 3.4σ 至 42.7σ；均场的高估随 m 减小、随 λ 增大。", { size: SMALL }),
  ] }));

// ---------------- 3.4 ----------------
k.push(H2("3.4. 数值校验"));
k.push(P([R("本节的结论依赖于两件事：均场侧的实现无误，以及精确侧确实是真值。二者分别校验如下。脚本见 verify_mf.py 与 verify_sim.py。")], { ind: false }));
k.push(lead("均场侧的内部一致。", [R("(3.2) 的矩阵与 (3.1) 的 ODE 是两条独立实现，其阈值一致：在 "), ...E("ρ(N^{MF})=1"), R(" 处 ODE 的无病态 Jacobian 谱横标为零，残差约 "), ...E("10^{−11}"), R("（"), ...E("m=2,3,4,6"), R("）。此处曾有一处数值陷阱：以前向差分构造该 Jacobian 会引入随 m 增大的 "), ...E("O(d)"), R(" 截断误差（"), ...E("Θ"), R(" 的曲率含 "), ...E("(m−1)(m−2)"), R("），在 "), ...E("m=6"), R(" 处即达 "), ...E("10^{−6}"), R("（步长 "), ...E("d=10^{−6}"), R("），比被检验的 "), ...E("10^{−7}"), R(" 判据还大一个量级；改用中心差分后降至 "), ...E("10^{−11}"), R("。此外还独立积分了 (3.1)：在 "), ...E("0.85λ_{c}^{MF}"), R(" 与 "), ...E("1.15λ_{c}^{MF}"), R(" 两侧分别给出亚临界与超临界行为，闭合了线性化与动力学之间的回路。")]));
k.push(lead("精确侧的真值。", [R("图 3 本身是两个闭合之间的比较，故其结论须有闭合以外的真值支撑。为此在位形模型多重超图上作精确的连续时间 Gillespie 仿真（含真实回路与有限 N，不用任何闭合）。取 "), ...E("N=4×10^{4}"), R("、3000 次播种（分摊于 3 张独立实现），在 "), ...E("λ=0.6λ_{c}"), R(" 与 "), ...E("0.8λ_{c}"), R("、"), ...E("m=3,5,8,12"), R(" 共八个点上，仿真的平均簇规模与精确理论一致至 "), ...E("1.3σ"), R("，而均场的预言被排除 3.4σ 至 42.7σ（图 3d）。这同时检验了 T、余度相减与级联三项。（"), ...E("N=1.2×10^{4}"), R(" 时其中一点偏离 3.1σ，增大 N 后降至 0.4σ，确认为有限尺寸效应。）")]));
